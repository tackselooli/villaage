...
# if you using UNIX OS u can just type "make [name of command u want run]" for e.g.  "make build"
# if using windows u have 3 options:  1. install chocolaty  2. install subsystem for linux  3. install GunWin
...
volume:
	docker volume inspect villaage_postgres_data

villaage-db:
	docker compose exec postgres-db psql --username=postgres --dbname=villaageDB
